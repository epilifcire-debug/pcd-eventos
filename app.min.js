const API_BASE=window.location.hostname.includes("localhost")||window.location.hostname.includes("127.0.0.1")?"http://localhost:3000":"https://pcd-eventos.onrender.com";
...
const loginScreen=document.getElementById("login-screen"),appSection=document.getElementById("app"),loginEmail=document.getElementById("login-email"),loginPass=document.getElementById("login-pass"),loginMsg=document.getElementById("login-msg"),btnLogin=document.getElementById("btn-login"),btnLogout=document.getElementById("btn-logout"),navAdmin=document.getElementById("nav-admin");let token=null,userData=null;
...
async function loadEventos(){
  ...
  try{
    ...
    if(!eventos.length){card.innerHTML="<h2>Eventos</h2><p>Nenhum evento cadastrado.</p>";return;}
    let html="<h2>Eventos</h2><table><thead><tr><th>Nome</th><th>Data</th><th>Descrição</th></tr></thead><tbody>";
...
if(savedToken&&savedUser){
  token=savedToken;userData=JSON.parse(savedUser);
  loginScreen.classList.add("hidden");
  appSection.classList.remove("hidden");
  if(userData.role==="admin")navAdmin.classList.remove("hidden");
  loadEventos();
